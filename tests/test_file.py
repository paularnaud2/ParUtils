...
def test_file():
    ...
    out = u.list_files('tests', only_list=['test'], ignore_list=['file', 'msc', 'init'])
    ...
    out = u.list_files('tests', only_list=['test'], ignore_list=['file', 'msc', 'init'], abspath=True)
    lst = ['c:/*/tests/test_csv.py', 'c:/*/tests/test_dq.py']
    for elt in out:
        assert u.like_list(elt, lst)
